...
public class Detector {
    ...
    public void findObject(Mat frame, List<MatOfPoint> contours, boolean bgOn) {
    ...
    private void findFace(CascadeClassifier detector, Mat frame) {    
        Mat grayFrame = new Mat();
        Imgproc.cvtColor(frame, grayFrame, Imgproc.COLOR_BGR2GRAY);
        Mat scaledFace = grayFrame;
        Imgproc.resize(scaledFace, scaledFace, new Size(scaledFace.width() * 2, scaledFace.height() * 2));
        ...
        String objectName = "Annonimus";
        for (Rect face : targets.toArray()) {

            if (isTrained) {
     
                Rect subFace = face.clone();
                subFace.x -= 30;
                subFace.y -= 30;
                subFace.width +=60;
                subFace.height+=60;
                
                objectName = recognizer.recognize(grayFrame.submat(subFace));

                JTextArea log = logSystem.getLog("recLog");
                log.append("Timestamp: " + mark[0] + " " + mark[1] + "    Face: " + objectName + "\n");
            }

            Imgproc.rectangle(frame, new Point(face.x / 2, face.y / 2),
                    new Point(face.x / 2 + face.width / 2, face.y / 2 + face.height / 2),
                    settings.RecognizerFaceLineColor, 1);
            logSystem.counter += 1;

            Imgproc.putText(frame, objectName, new Point(face.x/2, face.y/2 + face.height/2 + 25),
                    2, 1, new Scalar(255, 255, 0), 2);
        }
